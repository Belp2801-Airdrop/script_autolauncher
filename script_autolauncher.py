...
class ScriptAutoLauncher(customtkinter.CTk):
    ...
    def init_ctk_var(self):
        ...
        for script in self.scripts.keys():
            ...
            self.time_var[script].set(self.format_time(self.data[script]['remain_time']))
            self.success_count_var[script].set(str(self.data[script]['success_count']).rjust(2, '0'))
    # endregion
    ...
    def build_toolbar_frame(self):
        self.toolbar_frame = customtkinter.CTkFrame(self, corner_radius=10, )
        self.toolbar_frame.pack(fill=customtkinter.BOTH, expand=True, padx=5, pady=(5, 0))
        
        # combobox
        self.combobox_frame = customtkinter.CTkFrame(self.toolbar_frame, bg_color="transparent", fg_color="transparent")
        ...
        # function button
        self.function_frame = customtkinter.CTkFrame(self.toolbar_frame, bg_color="transparent", fg_color="transparent")
    ...
    def build_grid_frame(self):
        ...
        for row_index, row_data in enumerate(self.data_grid):
            for col_index, cell_data in enumerate(row_data):
                ...
                frame.grid(row=_row, column=_col, padx=(15, 0) if _col > 0 and _col % len(headers) == 0 else 0)
                if _row == 0:
                    ...
                    if col_index == headers.index("Func"):
                        entry = customtkinter.CTkButton(
                            ...
                            text="Reset", width=column_widths[col_index]
                        )
                    ...
                    # Time countdown
                        ...
                        entry = customtkinter.CTkEntry(
                            ...
                            justify=customtkinter.CENTER, width=column_widths[col_index],
    ...
    def save_lastest_running_time(self):
        values = {key: inner_dict["start_time"] for key, inner_dict in self.data.items()}
